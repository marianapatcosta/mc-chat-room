...
const { censuredWords } = require("./constants");
...
socketIo.on("connection", socket => {
  ...
  socket.on("sendMessage", (sentMessage, callback) => {
    ...
    socketIo.to(user.room).emit("message", generateMessage(user.username, sentMessage)); //emits event to all the connections
    callback();
  });
